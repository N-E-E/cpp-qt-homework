...
int file_widget::read_stop_file(QString file_name)
{
	...
	if (stop_file) {
		...
		std::cout << "stop file 打开错误！" << std::endl;
		return 0;
	}
}
...
int file_widget::read_line_file(QString file_name)
{
	...
	if (line_file) {
		...
		std::cout << "line file 打开错误！" << std::endl;
		return 0;
	}
}
...
void file_widget::check_file()
{
	...
	else if (this->read_stop_file(this->stop_file_name) && this->read_line_file(this->line_file_name)) {  // 文件正确，就把点和线打印出来,否则也是输出错误语句
		emit true_stop_file(this->points);
		emit true_line_file(stop_mat, this->line_num, this->line_of_points);
		this->close();
	}
	else {
		ui.CheckLabel->setText(QString::fromStdString("input file is empty or the directory has Chinese symbol!").toUtf8());  // to do.还需要解决乱码问题
	}
