...
public:
	...
	// 传入起点站和终点站以及两个站点所在的路线、路线信息，就可以构造同一条路线上的一条路
	Line(int start_stop, int end_stop, const GraphInfo& info) : \
		...
		for (int i = 0; i < target.size(); i++) {
			if (target[i] == start_stop) start_stop_ind = i;
			if (target[i] == end_stop) end_stop_ind = i;
		}
		if (start_stop_ind == -1 || end_stop_ind == -1) throw "stop ind error!";
